...
export async function GET(
  ...
  try {
    ...
    return NextResponse.json(
      { error: "Error al obtener la asignación", details: error.message },
      { status: 500 }
    );
  }
}
...
export async function PUT(
  ...
  try {
    ...
    if (!payload || (payload.role !== "Admin" && payload.role !== "S_A")) {
      // Only Admin or S_A can update assignments
      return NextResponse.json(
        {
          error:
            "Acceso denegado. Solo administradores pueden actualizar asignaciones.",
        },
        { status: 403 }
      );
    }
    ...
    if (truckId !== undefined) {
      if (isNaN(Number(truckId)))
        return NextResponse.json(
          { error: "truckId debe ser un número" },
          { status: 400 }
        );
    ...
    if (driverId !== undefined) {
      if (isNaN(Number(driverId)))
        return NextResponse.json(
          { error: "driverId debe ser un número" },
          { status: 400 }
        );
    ...
    if (totalLoaded !== undefined) {
      if (isNaN(Number(totalLoaded)))
        return NextResponse.json(
          { error: "totalLoaded debe ser un número" },
          { status: 400 }
        );
    ...
    if (totalRemaining !== undefined) {
      if (isNaN(Number(totalRemaining)))
        return NextResponse.json(
          { error: "totalRemaining debe ser un número" },
          { status: 400 }
        );
      updateData.totalRemaining = Number.parseFloat(totalRemaining);
    }
    if (fuelType !== undefined) {
      // Optional: Validate fuelType against FuelType enum if needed
      ...
      if (!validFuelTypes.includes(fuelType))
        return NextResponse.json(
          { error: "Tipo de combustible inválido" },
          { status: 400 }
        );
    ...
    if (isCompleted !== undefined) {
      if (typeof isCompleted !== "boolean")
        return NextResponse.json(
          { error: "isCompleted debe ser un booleano" },
          { status: 400 }
        );
    ...
    return NextResponse.json(
      { error: "Error al actualizar asignación", details: error.message },
      { status: 500 }
    );
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    const payload = await verifyToken(token);
    if (!payload || (payload.role !== "Admin" && payload.role !== "S_A")) {
      // Only Admin or S_A can delete assignments
      return NextResponse.json(
        {
          error:
            "Acceso denegado. Solo administradores pueden eliminar asignaciones.",
        },
        { status: 403 }
      );
    }
    ...
    return NextResponse.json(
      { error: "Error al eliminar asignación", details: error.message },
      { status: 500 }
    );
  }
}
